...
export async function sendMessage(
  ...
  try {
    ...
    await execFileAsync("supervisorctl", ["stop", "wacli-sync"], {
      timeout,
    });
    ...
    const response = wacliSendResponseSchema.parse(JSON.parse(stdout));
    ...
    try {
      await execFileAsync("supervisorctl", ["start", "wacli-sync"], {
        timeout,
      });
    } catch (restartError) {
